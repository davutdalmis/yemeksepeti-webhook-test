...
//   1. order.parasutQueued === true -> skip (marker)
...
class ProductionOrderListener {
    ...
    async _handleOrder(docSnap) {
        ...
        if (order.parasutQueued === true) return;
        ...
        // Kalem snapshot'ı — ProductionOrderItem: {productId, productName, quantity, unit, unitPrice, totalPrice}
        const itemsSnapshot = Array.isArray(order.items)
        ...
        const amount = itemsSnapshot.reduce((sum, it) => sum + it.originalQuantity * it.unitPrice, 0);
